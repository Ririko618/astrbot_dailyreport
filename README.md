...
| `theme` | 下拉选择 | `天依蓝` | 日报配色：天依蓝 / pink / purple / 初音绿 / orange |
| `api_token` | string | `""` | ALAPI Token，用于节假日、每日古诗词、早报 API。在 https://admin.alapi.cn/user/login 获取 |
...
2. **番剧 API**：主地址 `api.bgm.tv` 不可用时自动切换备用地址 `bgmapi.anibt.net`
3. **Playwright**：首次使用需安装 Chromium 浏览器
4. **角色导入**：必须包含至少一个 `.md` 人格文件，文件名决定角色名
